...
from .forms import WynikiModelForm, RejestracjaModelForm, TurniejModelForm, ModuleFormSet, ModuleFormSetDynamic, RejestracjaDynamicModelForm, WynikiDynamicModelForm, RejestracjaModelFormNew, RejestracjaCheckboxForm, RejestracjaDynamicCheckboxForm
...
def wyniki(request, pk):
	...
	if context['nazwa_turnieju'][0].wyniki_widoczne or request.user.rts:
		...
		for i in zawody_dynamic:
			...
			wynik = WynikiDynamic.objects.filter(zawody = i, oplata=1, kara='BRAK').exclude(czas=0).order_by('kara', 'wynik', 'czas')
			wynik_zero = WynikiDynamic.objects.filter(zawody = i, oplata=1, czas=0, kara='BRAK').order_by('kara', 'wynik', 'czas')
			wynik_kara = WynikiDynamic.objects.filter(zawody = i, oplata=1).exclude(kara='BRAK').order_by('kara', 'wynik', 'czas')
			union = wynik.union(wynik_zero)
			slownik['wyniki'] = list(chain(wynik, wynik_kara, wynik_zero))
	...
	if context['nazwa_turnieju'][0].wyniki_generalne_widoczne or request.user.rts:
		...
		for i in zawody_grup:
			...
			for num, j in enumerate(elements):
				# print(f'num: {num}')
				select_strings_to_add.append(f'case w{j}.oplata when 0 then "Nie startował" else coalesce(w{j}.wynik,"Nie startował") end w{num}')
				...
				from_strings_to_add.append(f" left join (select wynik, oplata, zawodnik_id from wyniki_wyniki where wyniki_wyniki.zawody_id = {j}) w{j} on w{j}.zawodnik_id = account_account.id")
...
################################################
		...
		for num, j in enumerate(turniej_grup_set):
				# print(f'num: {num}')
			select_strings_general.append(f'case w{j}.wynik when 0 then "Nie startował" else coalesce(w{j}.wynik,"Nie startował") end w{num}')
			# if context['nazwa_turnieju'][0].turniej_druzynowy:
...
def rejestracja_na_zawody_checkbox(request, pk):
	...
	# dodawanie_zawodnika jest podawane do templatki html i na podstawie tego w templatce jest wyswietlany formularz lub napis że rejestracja zamknieta
	context['dodawanie_zawodnika'] = nazwa_turnieju(pk)[0].rejestracja
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			#sprawdzamy jakie konkurencje są w danym turnieju i tworzymy słownik nazwa_konkurencji:id_konkurencji
			konkurencje_w_turnieju_queryset = Zawody.objects.filter(turniej__id = pk).values('nazwa','id')
			konkurencje_dict = {}
			for konkurencja in konkurencje_w_turnieju_queryset:
				konkurencje_dict[konkurencja['nazwa']] = konkurencja['id']

			# zbieramy dane z formularza do zmiennej cleaned_data
			cleaned_data = form.cleaned_data


			# jesli user to rts lub admin to powieramy wybranego użytkownika w formularzu, jeśli to zwykły user to do zmiennej zawodnik_ wstawiamy usera ktory jest zalogowany/sklada requesta
			if request.user.rts or request.user.is_admin:
				zawodnik_id = form.cleaned_data['Zawodnik']
				zawodnik_ = Account.objects.get(id = zawodnik_id)
			else:
				zawodnik_ = request.user

			# iterujemy po słowniku z nazwami konkurencji
			for konkurencja in konkurencje_dict:

				# jesli w danych zebranych w formularzu - cleaned_data - jest iterowana konkurencja
				if cleaned_data[konkurencja]:

					# pobieramy id konkurencji
					zawody_ = Zawody.objects.get(id = konkurencje_dict.get(konkurencja))
					# sprawdzamy czy dany zawodnik jest już zapisany do tej konkurencji
					juz_zapisany = Wyniki.objects.filter(zawodnik = zawodnik_, zawody = zawody_)

					# jesli nie jest to zapisujemy usera do konkurencji uprzednio sprawdzajac w cleaned_data czy zaznaczyl bron klubowa i amunicje klubowa
					if not juz_zapisany:
						bron_klubowa_string = f'{konkurencja} - broń klubowa'
						amunicja_klubowa_string = f'{konkurencja} - amunicja klubowa'
						bron_klubowa_ = cleaned_data[bron_klubowa_string]
						amunicja_klubowa_ = cleaned_data[amunicja_klubowa_string]
						Wyniki.objects.create(zawodnik = zawodnik_, zawody = zawody_, bron_klubowa= bron_klubowa_, amunicja_klubowa = amunicja_klubowa_)
						# print('dodano')
					else:
						# print('zawodnik juz zapisany')
						pass
				else:
					# print('no')
					pass
	...
	# sumujemy oplaty za udzial w konkurencji i za amunicję/bron
	lista_zarejestrowanych = Wyniki.objects.filter(zawodnik = request.user).filter(zawody__turniej__id = pk)
	context['lista_zarejestrowanych'] = lista_zarejestrowanych
	suma_oplat = 0
	for i in lista_zarejestrowanych:
		suma_oplat += i.zawody.oplata_konkurencja
	...
	context['form'] = form
	context['zawodnik_rts'] = request.user.rts
...
@csrf_exempt
@login_required(login_url="start")
def rejestracja_na_zawody_dynamic_checkbox(request, pk):
	...
	if request.method == 'POST':
		...
		form = RejestracjaDynamicCheckboxForm(request.POST, zawody_pk=pk)

		if form.is_valid():

			#sprawdzamy jakie konkurencje są w danym turnieju i tworzymy słownik nazwa_konkurencji:id_konkurencji
			konkurencje_w_turnieju_queryset = ZawodyDynamic.objects.filter(turniej__id = pk).values('nazwa','id')
			konkurencje_dict = {}
			for konkurencja in konkurencje_w_turnieju_queryset:
				konkurencje_dict[konkurencja['nazwa']] = konkurencja['id']
			...
			for konkurencja in konkurencje_dict:
				...
				if cleaned_data[konkurencja]:
					...
					# pobieramy id konkurencji
					zawody_ = ZawodyDynamic.objects.get(id = konkurencje_dict.get(konkurencja))
					# sprawdzamy czy dany zawodnik jest już zapisany do tej konkurencji
					juz_zapisany = WynikiDynamic.objects.filter(zawodnik = zawodnik_, zawody = zawody_)
					...
					if not juz_zapisany:
						...
						bron_klubowa_ = cleaned_data[bron_klubowa_string]
						amunicja_klubowa_ = cleaned_data[amunicja_klubowa_string]
						WynikiDynamic.objects.create(zawodnik = zawodnik_, zawody = zawody_, bron_klubowa= bron_klubowa_, amunicja_klubowa = amunicja_klubowa_)
						# print('dodano')
					else:
						# print('zawodnik juz zapisany')
						pass
		...
		form = RejestracjaDynamicCheckboxForm(zawody_pk=pk)
	else:
		form = RejestracjaDynamicCheckboxForm(zawody_pk=pk)


	# sumujemy oplaty za udzial w konkurencji i za amunicję/bron
	lista_zarejestrowanych = WynikiDynamic.objects.filter(zawodnik = request.user).filter(zawody__turniej__id = pk)
	context['lista_zarejestrowanych'] = lista_zarejestrowanych
	suma_oplat = 0
	for i in lista_zarejestrowanych:
		suma_oplat += i.zawody.oplata_konkurencja
