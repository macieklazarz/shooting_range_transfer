...
class RejestracjaCheckboxForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['Zawodnik'] = forms.ChoiceField(choices = self.qs, required = False)
        ...
        for i in zawody_qs:
            ...
            self.fields[nazwa] = forms.BooleanField(required = False)
            self.fields[bron_klubowa] = forms.BooleanField(required = False)
            self.fields[amunicja_klubowa] = forms.BooleanField(required = False)



        # if user.rts:
        #     self.fields['Zawodnik'].widget.Hidden()
...
class RejestracjaDynamicCheckboxForm(forms.Form):
    qs = Account.objects.annotate(full_name=Concat('nazwisko', Value(' '), 'imie', output_field=CharField())).values_list('id', 'full_name').order_by('full_name')
    # zawodnik = forms.ModelMultipleChoiceField(widget=forms.CheckboxSelectMultiple, queryset= Zawody.objects.filter(turniej__id=22))
    ...
    def __init__(self, *args, **kwargs):
        # print('start')
        pk = kwargs.pop('zawody_pk', None)
        super(RejestracjaDynamicCheckboxForm, self).__init__(*args, **kwargs)
